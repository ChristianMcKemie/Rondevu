...
var enums = {"EventType": {"ROOM_SIZE_5": 5, "ICE_CONNECTION_STATE_CONNECTED": 3}, "RequestField": {"MessageType": {"EVENT": "event"}, "CLIENT_TYPE": "client_type", "EventField": {"EVENT_TIME_MS": "event_time_ms", "ROOM_ID": "room_id", "EVENT_TYPE": "event_type", "FLOW_ID": "flow_id"}, "TYPE": "type", "EVENT": "event", "REQUEST_TIME_MS": "request_time_ms"}, "ClientType": {"UNKNOWN": 0, "ANDROID": 4, "DESKTOP": 2, "IOS": 3, "JS": 1}};
...
if (typeof module !== "undefined") {
    ...
    module.exports = {RTCPeerConnection: RTCPeerConnection, getUserMedia: getUserMedia, attachMediaStream: attachMediaStream, dettachMediaStream: dettachMediaStream, reattachMediaStream: reattachMediaStream, webrtcDetectedBrowser: webrtcDetectedBrowser, webrtcDetectedVersion: webrtcDetectedVersion, webrtcMinimumVersion: webrtcMinimumVersion, webrtcTesting: webrtcTesting, webrtcUtils: webrtcUtils};
} else {
    if (typeof require === "function" && typeof define === "function") {
        define([], function () {
            return {RTCPeerConnection: window.RTCPeerConnection, getUserMedia: getUserMedia, attachMediaStream: attachMediaStream, dettachMediaStream: dettachMediaStream, reattachMediaStream: reattachMediaStream, webrtcDetectedBrowser: webrtcDetectedBrowser, webrtcDetectedVersion: webrtcDetectedVersion, webrtcMinimumVersion: webrtcMinimumVersion, webrtcTesting: webrtcTesting, webrtcUtils: webrtcUtils};
        });
    }
}
;
...
var AppController = function (loadingParams) {
    ...
    this.confirmJoinDiv_ = $$(UI_CONSTANTS.confirmJoinDiv);
    ...
    this.createCall_(loadingParams);
    ...
    document.getElementById('confirm-join-button').onclick = function () {
        console.log(document.getElementById('confirm-join-button').innerHTML);

        switch (document.getElementById('confirm-join-button').innerHTML) {
            case "HANG UP":
                $("#hangup").click();
                break;
            case "START":
                document.getElementById('confirm-join-button').innerHTML = "STOP";
                socket.emit('randomstart', loadingParams.ronid, function (data) {
                    if (data === 'success') {
            ...
            case "STOP":
                ...
                socket.emit('randomstop', loadingParams.ronid, function (data) {
                    if (data === 'fail') {
                        console.log('fail');
                    }
                    if (data === 'success') {
                        document.getElementById('confirm-join-button').innerHTML = "START";
                    }
                });
                break;
            case "SKIP":
                if (!document.getElementById('tensecprofileshowdiv').classList.contains("hide")) {
                    document.getElementById('tensecprofileshowdiv').classList.add("hide");
                }

                socket.emit('randomskip', loadingParams.ronid, function (data) {
                    console.log(data);
                    if (data === "success") {
                        document.getElementById('confirm-join-button').innerHTML = "STOP";
                    }
    ...
    /*
     if (document.getElementById('confirm-join-button').innerHTML === "HANG UP") {
     document.getElementById('confirm-join-button').innerHTML = "START";
     $("#hangup").click();
     } else {
     
     if (document.getElementById('confirm-join-button').innerHTML === "START") {
     ...
     if (document.getElementById('confirm-join-button').innerHTML === "STOP") {
     document.getElementById('confirm-join-button').innerHTML = "START";
     
     console.log(loadingParams.ronid);
     ...
     if (document.getElementById('confirm-join-button').innerHTML === "SKIP") {
     if (!document.getElementById('tensecprofileshowdiv').classList.contains("hide")) {
     document.getElementById('tensecprofileshowdiv').classList.add("hide");
     }
    ...
    socket.on("randompairingskipped", function (data) {
        console.log(data);
        if (!document.getElementById('tensecprofileshowdiv').classList.contains("hide")) {
            document.getElementById('tensecprofileshowdiv').classList.add("hide");
        }
        document.getElementById('confirm-join-button').innerHTML = "STOP";
        this.displayStatus_("They skipped you");
        var that = this;
        setTimeout(function () {
            that.displayStatus_("");
        }, 5000);
    ...
    socket.on("randompairingprofile10sec", function (data) {
        document.getElementById('confirm-join-button').innerHTML = "SKIP";

        //clearInterval(intervalId);
        console.log(data);
        document.getElementById('tensecprofileshowdiv').classList.remove("hide");
        document.getElementById('tensecpic').style.backgroundImage = "url('/images/" + data.id + "/" + data.pic.pic1 + ".jpg')";
        document.getElementById('tensecname').innerHTML = data.name;
        x = 10;
        document.getElementById('tensectimer').innerHTML = x;
    ...
    socket.on("ranstoppedtimer", function (data) {
        this.displayStatus_(data.name + " has stopped the timer");
        setTimeout(function () {
            this.displayStatus_("");
        }.bind(this), 5000);
    }.bind(this));
    ...
    socket.on("randompairingcallstart", function (data) {
        document.getElementById('tensecprofileshowdiv').classList.add("hide");
        //clearInterval(intervalId);
        ...
        if (data.room) {
            ...
            data.ronid = loadingParams.ronid;
            this.finishCallSetup_(data);
        }
...
AppController.prototype.createCall_ = function (loadingParams_) {
    console.log(loadingParams_);
    this.call_ = new Call(loadingParams_);
    ...
    $('.findmatchesbutton').click(function () {
        if ($$('#findmatcheswindow').classList.contains('hide')) {
            $$('#findmatcheswindow').classList.remove('hide');
            $$('#randomizewindow').classList.add('hide');
            this.hangup_();
            this.call_.onerror("");
            this.call_.disconnectVideo();
        }
    }.bind(this));

    $('.randomizebutton').click(function () {
        if ($$('#randomizewindow').classList.contains('hide')) {
            this.call_.connectVideo();
            $$('#randomizewindow').classList.remove('hide');
            $$('#findmatcheswindow').classList.add('hide');
        }
    }.bind(this));
};
AppController.prototype.finishCallSetup_ = function (data) {
    console.log("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!count");


    this.room = data.room;
    this.ronid = data.ronid;
    this.userId = data.userId;
    this.call_.start(data.room);
    ...
    $("#confirm-join-button").click(function () {
        ...
        if (document.getElementById('confirm-join-button').innerHTML === "END TIMER") {
           // console.log('1'+" "+this.ronid+" "+ this.userId);
            socket.emit('randomstoppedtimer', {ronid: this.ronid, userId: this.userId}, function (data) {
                 // console.log('2');
                if (data === 'fail') {
                    console.log('fail');
                }
                if (data === 'success') {
                    document.getElementById('confirm-join-button').innerHTML = "HANG UP";
                }
            });
        }
    }.bind(this));
...
AppController.prototype.disconnectVideo_ = function () {
    console.log(this);
    this.call_.disconnectVideo();
};

AppController.prototype.connectVideo_ = function () {
    this.call_.connectVideo();
};

AppController.prototype.hangup_ = function () {
    console.log(this.loadingParams_.ronid);
    trace("Hanging up.");
    ...
    console.log(this);
    document.getElementById('confirm-join-button').innerHTML = "START";
    document.getElementById('confirm-join-div').classList.remove("hidden");
    this.show_(this.localVideo_);
    this.hide_(this.remoteVideo_);
    this.hide_(this.miniVideo_);
    refreshRandomizeHistory();
    $("#randomMatchHistory").removeClass("hide");
    $("#timer").addClass("hide");
    socket.emit('randomstop', this.loadingParams_.ronid, function (data) {
...
AppController.prototype.onRemoteHangup_ = function () {
    this.displayStatus_("They hung up");
    this.transitionToDone_();
    this.call_.hangup(true);
    document.getElementById('confirm-join-button').innerHTML = "START";
    document.getElementById('confirm-join-div').classList.remove("hidden");
    ...
    $("#timer").addClass("hide");
    $("#randomMatchHistory").removeClass("hide");
    socket.emit('randomstop', this.ronid, function (data) {
        if (data === 'fail') {
            console.log('fail');
        }
    });
...
AppController.prototype.dettachLocalStream_ = function () {
    trace("Dettaching local stream.");
    dettachMediaStream(this.localVideo_);
    this.displayStatus_("");
...
AppController.prototype.transitionToActive_ = function () {
    ...
    document.getElementById('mini-video').classList.remove("hidden");
    //document.getElementById('confirm-join-div').classList.add("hidden");
...
AppController.prototype.toggleFullScreen_ = function () {
    if (isFullScreen()) {
        ...
        this.confirmJoinDiv_.classList.remove("fullScreenConfirmJoin");
        $("#tensecprofileshowdiv").removeClass('tensecprofiledivfullscreen');
        $("#tensecprofileshowdiv").addClass('tensecprofilediv');
        $("#status-div").removeClass("status-div-Fullscreen");
        $("#status-div").addClass("status-div");
        this.videosDiv_.classList.remove("fullScreen");
        this.videosDiv_.classList.remove("fullScreen");
        this.icons_.classList.remove("iconsFullScreen");
        this.icons_.classList.add("icons");
    } else {
        trace("Entering fullscreen.");
        document.querySelector("svg#fullscreen title").textContent = "Exit fullscreen";
        document.body.requestFullScreen();
        this.confirmJoinDiv_.classList.add("fullScreenConfirmJoin");
        $("#tensecprofileshowdiv").addClass('tensecprofiledivfullscreen');
        $("#tensecprofileshowdiv").removeClass('tensecprofilediv');
        $("#status-div").removeClass("status-div");
        $("#status-div").addClass("status-div-Fullscreen");
        this.videosDiv_.classList.add("fullScreen");
        this.icons_.classList.add("iconsFullScreen");
        this.icons_.classList.remove("icons");
...
AppController.prototype.showIcons_ = function () {
    if (!this.icons_.classList.contains("active")) {
        this.icons_.classList.remove("hideIcons");
...
AppController.prototype.hideIcons_ = function () {
    if (this.icons_.classList.contains("active")) {
        ...
        this.icons_.classList.add("hideIcons");
    }
};
...
Call.prototype.disconnectVideo = function () {
...
Call.prototype.connectVideo = function () {
...
Call.prototype.connectToRoom_ = function (roomId) {
    ...
    var channelPromise = this.channel_.open().catch(function (error) {
        this.onError_("Failed to open WebSocket");
    ...
    var joinPromise = this.joinRoom_().then(function (roomParams) {
        ...
        this.onError_("Failed to join room server");
        return Promise.reject(error);
    }.bind(this));
    Promise.all([channelPromise, joinPromise]).then(function () {
        ...
        Promise.all([this.getTurnServersPromise_, this.getMediaPromise_]).then(function () {
            ...
            this.onError_("Failed to start signaling");
        }.bind(this));
    }.bind(this)).catch(function (error) {
        this.onError_("Failed to register WebSocket");
    }.bind(this));
...
Call.prototype.maybeGetMedia_ = function () {
    ...
    if (needStream) {
        ...
        mediaPromise = requestUserMedia(mediaConstraints).then(function (stream) {
            ...
            this.onError_("No Camera Found.<br> Close any application that might be using the camera");
...
Call.prototype.maybeGetTurnServers_ = function () {
    ...
    if (shouldRequestTurnServers) {
        turnPromise = requestTurnServers(this.params_.turnRequestUrl, this.params_.turnTransports).then(function (turnServers) {
            ...
            if (this.onstatusmessage) {
                this.onstatusmessage("No TURN server; unlikely that media will traverse networks.");
            }
            trace(error.message);
        }.bind(this));
    } else {
...
Call.prototype.maybeCreatePcClient_ = function () {
    ...
    try {
        ...
        this.onError_("WebRTC is not supported by this browser. Try again with Firefox and Chrome");
    ...
    return new Promise(function (resolve, reject) {
        ...
        sendAsyncUrlRequest("POST", path).then(function (response) {
            ...
            if (responseObj.result !== "SUCCESS") {
                //  reject(Error("Registration error: " + responseObj.result + " " + path));
                return;
            }
...
PeerConnectionClient.prototype.onIceConnectionStateChanged_ = function () {
    ...
    if (this.pc_.iceConnectionState === "connected") {
        ...
        document.getElementById('confirm-join-button').innerHTML = "END TIMER";

        console.log(this);
        y = 20;
        document.getElementById('timer').innerHTML = "1:00";
        document.getElementById('timer').style.color = "rgb(76, 175, 80)";
        $("#timer").removeClass("hide");
        $("#randomMatchHistory").addClass("hide");
        var int2 = setInterval(function () {
            ...
            if (y >= 10) {
                document.getElementById('timer').innerHTML = "0:" + y;
            }
            if (y <= 9) {
                document.getElementById('timer').innerHTML = "0:0" + y;
            }
...
RoomSelection.prototype.buildRecentRoomList_ = function (recentRooms) {
    ...
    for (var i = 0; i < recentRooms.length; ++i) {
        ...
        href.href = location.origin + "/" + encodeURIComponent(recentRooms[i]);
        li.appendChild(href);
...
function sendAsyncUrlRequest(method, url, body) {
    return sendUrlRequest(method, url, true, false, body);
}

function sendUrlRequest(method, url, async, turn, body) {
    return new Promise(function (resolve, reject) {
        var xhr;
...
function requestTurnServers(turnRequestUrl, turnTransports) {
    ///console.log(turnRequestUrl);
    //turnRequestUrl = "https://therondevu.com:3478/turn";
    console.log(turnRequestUrl);
    return new Promise(function (resolve, reject) {
...
function refreshRandomizeHistory() {
    $.post("/refreshrandomizehistory", null, function (history, status) {
        console.log(history);
        var ranHist = "";
        for (var x = 0; x < history.length; x++) {
            ranHist += "<span matchprofileid='" + history[x].userId + "' style='";
            if (history[x].like === true) {
                ranHist += "border:3px solid #56bc76;";
            }
            ranHist += "background-image:url(\"images/" + history[x].userId + "/" + history[x].pic1 + ".jpg\");' name='" + history[x].name + "' class='pointer ranHistory'></span>";
        }
        console.log(ranHist);
        $('#randomMatchHistory').html(ranHist);
    });
}
